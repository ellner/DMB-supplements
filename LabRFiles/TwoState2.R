...
# TwoState2.R simulates a group of particles moving 
...
# and a large number of particles, the process should be approximated
# by a two-compartment differential equation
#
#   dQ1/dt= -R*Q1 + L*Q2      dQ2/dt= -L*Q2 + R*Q11
#
# Simulations of the Markov Chain are plotted along with solutions
# of the differential equation using R's ode function.  
...
while(t<tmax) {
   # how many move from L to R?
    moveR=ifelse(nL>0, rbinom(1,nL,Rdt), 0);
   # how many move from R to L?
    moveL=ifelse(nR>0, rbinom(1,nR,Ldt), 0);
   # make the changes
     nL=nL + moveL - moveR;
     nR=nR + moveR - moveL;
...
out=ode(y=c(nL0,nR0),times=seq(0,tmax,by=dt),func=diffus2,parms=0);

# plot stochastic simulation and deterministic solutions
par(bg="ivory2",cex.axis=1.25,cex.lab=1.25);  
matplot(tvals,cbind(ntL,ntR),type="l",lty=1,lwd=2,ylim=c(0,ntot), xlab="time",ylab="Number of Particles")
